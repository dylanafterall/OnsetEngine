...
/*
    1) store camera data
    2) store light data and render point lights & spot lights
    3) render skybox
    4) render game objects (with material comp)
    5) render sprites
    6) render stencil outlines
*/
...
void RenderSystem::update(
    ...
    // _________________________________________________________________________
    // -------------------------------------------------------------------------
    // 2) iterate over light entities for light data and shadow map rendering
    // _________________________________________________________________________
    // -------------------------------------------------------------------------
    int fbWidth;
    int fbHeight;
    ...
    lights.each([&](
        ...
        if (light.m_type == 0) {
            ...
            glm::vec3 testPos = glm::vec3(0.0f, 15.0f, 0.0f);
            lightView = glm::lookAt(testPos, glm::vec3(0.0f), glm::vec3(0.0, 1.0, 0.0));
            lightSpaceMatrix = lightProjection * lightView;

            // render scene from light's point of view
            glUseProgram(lightShader.m_depthMapProgram);
            glUniformMatrix4fv(glGetUniformLocation(lightShader.m_depthMapProgram, "lightSpaceMatrix"), 1, GL_FALSE, &lightSpaceMatrix[0][0]);
            glViewport(0, 0, m_shadowWidth, m_shadowHeight);
            glBindFramebuffer(GL_FRAMEBUFFER, shadow.m_shadowFramebuffer);
            ...
            lights.each([&](
                ...
                glUniformMatrix4fv(glGetUniformLocation(lightShader.m_depthMapProgram, "model"), 1, GL_FALSE, &ltModel[0][0]);
            ...
            objects.each([&](
                ...
                glUniformMatrix4fv(glGetUniformLocation(lightShader.m_depthMapProgram, "model"), 1, GL_FALSE, &objModel[0][0]);
    ...
    // _________________________________________________________________________
    // -------------------------------------------------------------------------
    // 4) iterate over game objects/entities to render to screen
    // _________________________________________________________________________
    // -------------------------------------------------------------------------
    objects.each([&](
        const auto& material,
